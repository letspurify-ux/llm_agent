...
\`\`\`
  ...
  '쿼리 실행 이력'의 결과 하나를 그대로 그릴 때는 표를 옮겨 적지 말고 \`data: step N\` 한 줄을 쓴다(N은 그 이력 항목의 번호 — "2. 쿼리이름 …"이면 2). 서버가 그 실행의 전체 결과(이력에 앞부분만 보인 것도 전부)로 표를 채우므로 옮겨 적는 것보다 정확하다. 이때도 type·title과 필요하면 x·y·y2를 함께 적는다. 결과가 문자열 위주의 목록·상세이면 차트를 쓰지 않는다.
