...
import { MAX_PROMPT_ITEM_LEN, MAX_DOC_LEN, indentLines } from './constants.js';
...
//   4: 청크별 trim을 하지 않는다. 내부 공백을 지우면 실제 겹침 길이를 잃어
//      반복 본문을 삭제하거나 짧아진 겹침을 중복해서 병합한다. 전체 문서 양끝만 trim한다.
export const CHUNK_SPLIT_VERSION = 4;
...
// 읽으면 실제 본문을 지운다. v4의 겹침은 150~151자다. 이전 청크와 외부 호출도 방어한다.
...
//
// 새 청크는 공백을 보존하여 겹침 길이가 고정된다. 문자열 대조는 서로게이트 보정과
// 이전 규칙의 청크도 받기 위해 남긴다. 가장 긴 것부터 찾아 짧은 우연이 긴 겹침을 가리지 않게 한다.
//
// 남은 앞머리는 원문 그대로 돌려준다. 새 청크는 공백까지 보존하므로 실제 겹침이
// CHUNK_OVERLAP(서로게이트 보정 시 +1)로 고정된다. 같은 문장을 여러 번 쓰거나 빈 줄이
// 길어도 서로 다른 본문을 겹침으로 지우지 않는다. 구 분할 규칙의 청크는 다음 동기화에서 갱신된다.
// 호출부가 legacy/외부 청크도 받을 수 있으므로 짧은 일치를 막는 기존 방어는 유지한다.
//
...
export function splitContent(text, {
  ...
  while (at < s.length) {
    ...
    // 내부 공백은 위치 정보다. 꼬리도 다른 청크와 같은 방식으로 보존한다.
  ...
  // 공백만 든 중간 청크도 보존한다. 걸러내면 이웃 번호가 실제로 인접하지 않게 되어
  // 병합 시 공백이나 본문이 사라진다. 비어 있는 문서는 위의 짧은 글 경로에서 처리한다.
  return out;
}
