...
// 최악(네 섹션이 예산에 꽉 찬 요청 + 대화 + 질문 + 시스템 = 81k자) ≈ 45k 토큰, 비우호적
// 토크나이저라도 ≈ 54k → 128k의 35~42%. 출력 가드(MAX_COMPLETION_TOKENS)를 더해도 절반 남짓이다.
...
export const PROMPT_FLOORS = {
  ...
  history: 23_800,   // MAX_HISTORY_ROWS 줄이 '어떤 조합으로 오든' 각자 상한까지 차도 전부 실리는 크기 —
                     ...
                     // 23,400이던 것을 올렸다 — 그 값은 '쿼리 5 + 안내 1 + 검색 3'이라는 한 조합만
                     // 견뎠고, 조합 무관 상한(23,557)에는 157자 모자랐다. 모자란 만큼은 아래에서 뗀다.
  queries: 19_600,   // 등록 30건(agent.js MAX_PROMPT_QUERIES)이 자세한 형태로 ~600자씩 — 남는 여유도 전부 여기로.
                     // 400자를 이력에 내주었다: 짧은 형태 보장(llm-openai.js renderQueries)이 이미 '등록한
                     // 쿼리가 통째로 사라지는 일은 없다'를 지키므로, 여기서 줄어드는 것은 자세한 줄 한둘뿐이고
                     // 그 쿼리는 지목하면 다음 스텝에 자세히 실린다. 이력에는 그런 복구 경로가 없다.
};
...
// 한 요청의 본문 청구 '행동' 횟수와, 한 문서가 지식 섹션에서 차지할 수 있는 글자 상한.
// 둘의 곱(9,000자)이 지식·처리방법 섹션의 최소 몫(PROMPT_FLOORS 각 10,000자) 안에 들어야 한다 —
// 둘 다 한 섹션에 몰려도 다른 후보 몇 건이 함께 실릴 자리가 남는다. 이 곱을 키우려면 그 몫부터 다시 본다.
//
// '항목 수'가 아니라 '행동 횟수'인 이유: 청크 구조에서 expand는 항목을 새로 펼치는 것이 아니라
// 이미 있는 항목의 '범위를 넓히는' 일이다(chunk.js buildItems, agent.js applyExpand). 항목 수로 세면
// 같은 항목을 두 번 넓히는 것이 한 개로 세어져 상한이 걸리지 않고, 그 사이 프롬프트만 계속 커진다.
