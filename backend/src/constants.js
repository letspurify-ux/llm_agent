...
// 섹션 본문이 아닌 고정 틀의 몫 — 섹션 제목 줄(건수·버림 표기 포함)·블록 사이 빈 줄·질문 제목·지시 블록
// (현재 시각 한 줄과 지시문. 남은 검색·청구·조회·읽기 수를 알리는 줄이 상황에 따라 넷까지 붙는다).
// 실측 최대치에 여유를 더해 잡는다 (llm-openai buildPrompt, 회귀 테스트가 실측한다).
// 이 몫을 떼지 않으면 네 섹션이 각자 예산에 꽉 찬 요청에서 정확히 이 길이만큼 전체 상한을
// 넘는다 — 회귀 테스트가 그 상태를 만들어 잡아내지만, 값이 어긋나면 그 전까지는 조용하다.
//
// 400이던 것을 올렸다. 그 값은 실제로 어긋나 있었다(실측 407, 건수·버림 표기까지 최대면 425):
// 재던 회귀 테스트가 agent가 만드는 ctx와 다른 모양을 재고 있었기 때문이다 — ① 자료 섹션이 한둘만
// 보이는 ctx로 재면서 '(없음)' 다섯 줄을 빼, 보이지도 않은 제목 줄 셋을 함께 빼버렸고 ② agent가 늘
// 넘기는 resultReadsLeft(조회 결과 추가 읽기 남음 줄)를 어느 모양에도 넣지 않았다. 그래서 틀이 커지는
// 변경(19회차의 남은 수 안내 줄들)이 들어와도 그 테스트는 통과했다. 지금은 다섯 섹션이 모두 보이는
// ctx로 지시 블록의 모든 조합을 재고 그중 최대를 본다.
export const PROMPT_FRAME_RESERVE = 500;
