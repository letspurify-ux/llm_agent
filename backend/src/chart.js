...
// (100행 × 8열 × 짧은 셀이면 5k 안팎. 조회 상한 MAX_ROWS(1000행)를 다 실으면 짧은 셀로도 이 예산을 넘으므로
//  그때는 행을 잘라 싣고 차트 아래에 그 사실을 적는다 — 프런트도 100행까지만 그린다, frontend/src/chart.js MAX_CHART_ROWS).
...
function splitBlock(body) {
  ...
  return { config, lines, hasTable: lines.some(l => l.table) };
}
...
export function resolveChartData(answer, steps) {
  ...
  return text.replace(FENCE_RE, (whole, indent, body) => {
    ...
    // 되돌아갈 때 남을 이유가 없다).
    const kept = lines.filter(l => l.key !== 'data').map(l => l.raw);
    ...
    if (budget <= 0) return note(config, '답변에 실을 수 있는 표의 양을 넘었습니다', indent);
    ...
    // 예산에 걸려 다 싣지 못한 표는 그 사실을 차트 아래 밝힌다 — 그래프만 보면 그것이 전부로 읽힌다.
