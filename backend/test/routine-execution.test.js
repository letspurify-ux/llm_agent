...
for (const queryType of ['PROCEDURE', 'FUNCTION']) for (const readOnly of [undefined, '0']) for (const mode of ['success', 'empty', 'fetch-error', 'abort', 'null-cursor', 'cursor-close-error', 'rollback-error', 'release-error', 'all-errors']) {
  test(`${queryType} 커서 ${mode}, READ_ONLY=${readOnly ?? '기본값'}: 제한된 fetch와 커서 닫기·rollback·연결 반납`, async t => {
    ...
    assert.deepEqual(calls.filter(c => c[0] === 'execute'), [
      ...(readOnly === '0' ? [] : [['execute', 'SET TRANSACTION READ ONLY']]), ['execute', sql],
    ]);
    if (mode !== 'null-cursor') {
