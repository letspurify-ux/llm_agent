...
// 스프레드시트가 수식으로 읽는 첫 글자(= @ 탭 CR, 그리고 숫자가 따르지 않는 + -)로 시작하는 문자열은
// 앞에 '를 붙여 글자로 고정한다 — DB의 자유 텍스트(VOC 본문 등)가 파일을 연 사람의 PC에서 수식으로
// 실행되지 않게. 숫자 셀은 문자열이 아니므로 -5 같은 값은 건드리지 않는다.
const FORMULA_START = /^(?:[=@\t\r]|[+-](?![\d.]))/;
const guardFormula = (v, s) => typeof v === 'string' && FORMULA_START.test(s) ? `'${s}` : s;
