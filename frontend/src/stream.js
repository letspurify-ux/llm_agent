...
// 응답 본문을 읽는다. 진행 이벤트마다 onEvent를 부르고, 첫 종료 이벤트(done/error/예전 JSON)를 돌려준다.
...
export async function readEvents(res, onEvent) {
  // JSON 하나는 줄 단위 프로토콜이 아니다. 들여쓴 응답도 하나의 객체로 읽는다.
  const contentType = res.headers?.get?.('content-type')?.split(';')[0].trim().toLowerCase();
  if (contentType === 'application/json') return eventOf(await res.text());
  let final = null;
  const feed = events => {
    for (const e of events) {
      if (final) return;
      if (isFinal(e)) { final = e; return; }
      try { onEvent?.(e); } catch (err) { console.warn('[stream] 진행 표시에 실패했습니다:', err); }
    }
  };

  // NDJSON 줄 나누기는 한 곳뿐이다. 스트림으로 오는 길과 통째로 오는 길(본문 스트림이 없는 환경)이
  ...
  if (res.body?.getReader) {
    ...
    try {
      for (;;) {
        ...
        // done/error가 도착하면 답은 완성됐다. 프록시가 연결을 계속 열어 두어도 기다리지 않는다.
        if (final) return final;
      }
      feed(take(decoder.decode()));
    } finally {
      // 취소의 완료를 기다리면 응답하지 않는 연결에 다시 매일 수 있다.
      if (final) reader.cancel().catch(() => {});
      reader.releaseLock();
    }
  } else {
