  ...
  useEffect(() => {
    ...
    if (!growRef.current) {
      growRef.current = new ResizeObserver(entries => {
        ...
        // 바닥에 와 있으면 붙은 것으로 되돌린다. onChatScroll이 스크롤마다 세우는 규칙('바닥에
        // 닿으면 붙는다')과 같은 것인데, 내용이 '줄어드는' 변화는 scrollTop을 옮기지 않아 스크롤
        // 이벤트가 나지 않는다 — 그 한 길만 이 규칙 밖에 있었다.
        // 실측: 바닥에서 답을 기다리는 동안 '표로 보기'를 폈다 접으면(펼침이 뗀 것을 접힘이
        // 되돌리지 않는다) 화면은 다시 바닥인데 떨어진 채로 남아, 도착한 답의 1,030px이 화면 밖에
        // 그대로 있었다. 접힘에서 떼지 않기로 한 이유가 바로 그 손해였는데(아래 onToggle 주석)
        // 정작 그 손해가 한 걸음 뒤에서 그대로 일어나고 있었다.
        // 자라는 변화에서는 걸리지 않는다 — 자란 뒤에는 바닥까지의 거리가 이미 벌어져 있다.
        // 그래서 '펼치면 그 자리에 머문다'는 규칙은 그대로다.
