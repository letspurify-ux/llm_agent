...
// \(…\)·\[…\] 한 덩어리의 길이 상한. 닫는 표시를 끝까지 찾는 비탐욕 매치는 닫히지 않은 여는 표시가 많은
// 퇴화한 답변에서 '여는 표시 수 × 길이'가 된다(실측: 답변 상한 안의 '\( x ' 되풀이가 319ms, 길이 두 배에 네 배 —
// 미리보기는 120ms마다 자라난 전체를 다시 그리므로 그 비용이 스트림 끝까지 쌓인다). 그래서 한 덩어리가 찾아보는
// 길이를 여기서 묶는다 — 5,000자보다 긴 수식 하나는 조판되지 않고 원문으로 남는다(실패 방향은 그쪽이다). 답변
// 하나의 상한(서버 MAX_ANSWER_LEN 70,000자)에서 최악의 비용은 '여는 표시 수 × 5,000'이라 100ms 안쪽이다.
const MAX_SPAN = 5000;
const SPAN_RE = new RegExp(`\\\\\\(([\\s\\S]{1,${MAX_SPAN}}?)\\\\\\)|\\\\\\[([\\s\\S]{1,${MAX_SPAN}}?)\\\\\\]|\\$([^$\\s]|[^$\\s][^$]*[^$\\s])\\$`, 'g');
...
// 길이 상한은 위 SPAN_RE와 같다 — 같은 덩어리를 한쪽만 다르게 재면 문장 속 표기와 별행 판정이 갈린다.
const BRACKET_ALL_RE = new RegExp(`\\\\\\[([\\s\\S]{1,${MAX_SPAN}}?)\\\\\\]`, 'g');
