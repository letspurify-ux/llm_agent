...
쿼리는 query_name 정확 일치를 먼저 찾고, 벡터 검색 결과와 중복 없이 합쳐 최소 3건을 보충합니다.
정확히 일치한 쿼리를 첫 번째에 두고 나머지는 가까운 순서로 반환하며, 벡터 검색이 불가능해도 정확 적중은 보존합니다.
...
정확한 쿼리명 검색도 해당 쿼리를 포함해 최소 3개까지 보충한다. `SEARCH_LIMIT`를 1이나 2로 설정해도 최소값 3을 적용한다.
