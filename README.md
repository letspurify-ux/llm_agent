...
답변의 LaTeX 수식을 KaTeX로 그린다. **표기는 `$$` 하나뿐이다.**

| 쓰는 법 | 결과 |
|---|---|
| 한 줄에 `$$E=mc^2$$` | 문장 안에 들어가는 인라인 수식 |
| `$$` 를 **앞뒤 각각 독립된 줄**에 두고 그 사이 | 가운데 정렬된 별행 수식. 말풍선보다 넓으면 그 안에서 가로 스크롤된다 |
| `$E=mc^2$` (달러 하나) | **수식이 아니다.** 원문 그대로 나온다 |
| `\(...\)`, `\[...\]` | 수식이 아니다. 원문 그대로 나온다 |

**달러 하나를 수식으로 보지 않는 것은 의도된 선택이다.** 켜 두면 본문에 `$`가 둘만 있어도 그
사이가 통째로 수식이 되어 문장이나 표가 깨진다 — 실측으로 `| 배송 | $100 |` / `| 상품 | $200 |`
두 행이 하나의 수식으로 먹혀 표가 무너졌다. 조회 결과를 표로 보여주는 것이 이 앱의 본업이라
그 손상이 수식 표기의 편의보다 크다. 덕분에 금액(`$100`)·환경변수(`$ORACLE_HOME`)는 아무 처리
없이 그대로 써도 안전하다. 되돌리려면 `App.jsx`의 `singleDollarTextMath: false` 를 지운다.

알고 있어야 할 두 가지.

- **넓은 수식은 반드시 별행(`$$` 독립 줄)으로 쓴다.** 별행 수식은 말풍선 안에서 가로 스크롤되지만
  **인라인 수식은 접히지도 스크롤되지도 않는다** — KaTeX가 `.base`에 `white-space: nowrap`을 걸어
  한 덩어리로 다루기 때문이다. CSS로 푸는 세 방법(`inline-block`+`overflow`, 문단 `overflow-x`,
  `.text`의 `white-space`)을 모두 재봤고 각각 기준선 5px 어긋남 / 분수 세로 잘림 / 한 글자씩
  줄바꿈을 일으켜 채택하지 않았다. 대신 `.bubble`의 `overflow-x: clip`이 넘치는 부분을 상자 안에
  가둔다(잘려서 안 보일 뿐 말풍선을 뚫지는 않는다).
- **`answer`는 JSON 문자열이라 백슬래시를 두 번 써야 한다** (`\\frac`). 한 번만 쓰면 `\[` 처럼
  JSON에 없는 이스케이프는 응답 전체가 버려지고, `\f`·`\b` 처럼 JSON에 있는 이스케이프는
  `\frac`·`\beta` 가 제어문자로 조용히 바뀐다. 시스템 프롬프트가 이를 지시하고,
  `llm-openai.js` 가 두 경우 모두 복구한다 (복구 불가능한 `\t`·`\n`·`\r` 만 프롬프트에 의존).

표기를 바꾸려면 프론트엔드(`App.jsx`의 remark-math/rehype-katex)와 백엔드 시스템 프롬프트를
**함께** 바꿔야 한다 — 한쪽만 바꾸면 수식이 원문 그대로 화면에 노출된다.
