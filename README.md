...
두 경로의 합집합만 싣는다 — 등록이 적어도 검색에 걸리지 않은 나머지를 얹지 않는다(무관한 질문에 등록 쿼리 전부가
후보로 실려 모델이 억지로 하나를 고르던 자리를 막았다). 정렬이 규모와 무관해야 하는 이유는 프롬프트 예산이 "뒤쪽일수록 덜 관련됐다"는 전제로 뒤에서부터
줄이기 때문이다.
...
즉 **한 검색이 올린 쿼리는 짧은 줄로라도 반드시 실린다**. (목록은 검색마다 병합되어 커지므로, 세 검색이 서로 다른
30건씩 쌓는 극단에서만 가장 오래된 검색의 꼬리가 생략 안내와 함께 빠진다 — `context.md` 3-1.)
