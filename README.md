...
첫 Markdown 파싱은 코드·주소·문단·표 행의 경계를 제공한다. 그 좌표로 원문에서 수식을 추출해
보호한 뒤 Markdown을 다시 파싱한다. 수식 안의 `|`·`_`·`*` 때문에 표가 갈라지거나 수식이
...
보호하고 KaTeX에서 검증한다. 표에서 닫는 구분자가 없는 명확한 TeX 입력은 `table-math.js`가
