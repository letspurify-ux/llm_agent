...
루틴은 기본적으로 읽기 전용 트랜잭션으로 실행한다. 내부에서 DML이 필요한 루틴은
`backend/.env`에 아래 값을 설정하고 백엔드를 재시작하면 `SET TRANSACTION READ ONLY`를 생략한다.

```dotenv
ORACLE_ROUTINE_READ_ONLY=0
```

`1`(기본값)이면 읽기 전용이며, `true/yes/on`과 `false/no/off`도 지원한다(대소문자 무관).
미설정·빈 값은 기본값을 사용하고, 잘못된 값은 경고를 남기고 읽기 전용을 유지한다.
이 설정은 등록된 PROCEDURE·FUNCTION 전체에 적용하며, QUERY의 SELECT/WITH 전용 검사는 유지한다.
DB 계정 권한은 별도로 필요하며, 설정과 관계없이 `autoCommit: false`로 실행하고 결과를 읽은 뒤 rollback한다.
따라서 읽기 전용 해제는 자동 커밋을 켜는 설정이 아니다. 루틴 내부에서 명시적으로 커밋하거나
자율 트랜잭션·외부 호출로 수행한 변경은 마지막 rollback으로 취소되지 않을 수 있다.
